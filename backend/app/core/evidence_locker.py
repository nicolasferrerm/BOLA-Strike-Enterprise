...
class EvidenceLocker:
    ...
    def store_evidence(self, idx: int, result: Dict[str, Any]):
        ...
        with open(
            os.path.join(self.output_dir, fname), "w", encoding="utf-8", newline="\n"
        ) as f:
        ...
        self.manifest_entries.append(
            {
                ...
                "timestamp": ts,
                "severity": severity,
                "endpoint": f"{result.get('method')} {result.get('path')}",
            }
        )
    ...
    def write_manifest(self, scan_meta: Dict[str, Any] = None) -> str:
        ...
        manifest = {
            "schema_version": "1.0",
            "tool": "BOLA Strike Enterprise",
            "tool_version": __version__,
            "generated_at": datetime.datetime.now(datetime.timezone.utc).isoformat(),
            "chain_of_custody": {
                "operator_user": user,
                "operator_host": platform.node(),
                "operator_os": f"{platform.system()} {platform.release()}",
            },
            "scan_metadata": scan_meta or {},
            "evidence_count": len(self.manifest_entries),
            "evidence": self.manifest_entries,
        }
...
def preserve_evidence(
    results: List[Dict[str, Any]], target_url: str = "", output_dir: str = "evidence"
) -> str:
    locker = EvidenceLocker(output_dir=output_dir)
    for idx, result in enumerate(results):
        locker.store_evidence(idx, result)
    return locker.write_manifest(
        scan_meta={
            "target_url": target_url,
            "total_scanned": len(results),
            "critical_high": sum(
                1 for r in results if r.get("severity") in ("CRITICAL", "HIGH")
            ),
        }
    )
