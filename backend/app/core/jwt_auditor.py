...
def audit_jwt(token: str, wordlist_path: str = None) -> Dict[str, Any]:
    ...
    if len(parts) == 5:
        return {
            "valid_jwt": False,
            "is_jwe": True,
            "error": "Token is a JWE (Encrypted). Cannot audit claims offline.",
            "findings": [],
        }
    if len(parts) != 3:
        return {
            "valid_jwt": False,
            "is_jwe": False,
            "error": f"Not a valid JWT (expected 3 segments, got {len(parts)})",
            "findings": [],
        }
    ...
    if not header or not payload:
        return {
            "valid_jwt": False,
            "is_jwe": False,
            "error": "Failed to decode JWT header or payload",
            "findings": [],
        }
        ...
        findings.append(
            {
                ...
                "detail": f"JWT uses algorithm '{alg}'. An attacker can forge tokens without a signature.",
        ...
        findings.append(
            {
                "id": "JWT-006",
                "severity": "HIGH",
                "title": "External Key Injection Surface",
                "detail": "Header contains jku/jwk/x5u. If the server does not strictly validate the URI/Key, attackers can force the server to trust their own public key.",
        ...
        findings.append(
            {
                "id": "JWT-007",
                "severity": "LOW",
                "title": f"Non-standard 'typ' header: {typ}",
                "detail": "The typ header should explicitly be 'JWT' to prevent cross-JWT confusion attacks (e.g., at+jwt).",
                ...
                findings.append(
                    {
                        "id": "JWT-002",
                        "severity": "CRITICAL",
                        "title": "HMAC Key Cracked",
                        "detail": f"The JWT signature was cracked using a weak key: '{key}'.",
            ...
            findings.append(
                {
                    "id": "JWT-002",
                    "severity": "INFO",
                    "title": f"HMAC Algorithm ({alg})",
                    "detail": "HMAC tokens are vulnerable to offline brute-force. Key was not found in dictionary.",
                    "cwe": "CWE-326",
                }
            )
            ...
            findings.append(
                {
                    "id": f"JWT-003-{claim}",
                    "severity": "LOW",
                    "title": f"Missing Security Claim: '{claim}'",
                    "detail": f"The JWT payload is missing the '{claim}' claim, reducing validation robustness.",
                ...
                findings.append(
                    {
                        "id": "JWT-004",
                        "severity": "HIGH",
                        "title": "Token is Expired (Replay Attack Surface)",
                        "detail": f"Token expired at {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(exp))}. If the API still accepts this token, it is vulnerable to replay attacks.",
    ...
    for claim_key in payload.keys():
        if any(pattern in claim_key.lower() for pattern in SENSITIVE_CLAIM_PATTERNS):
            findings.append(
                {
                    "id": f"JWT-005-{claim_key}",
                    "severity": "MEDIUM",
                    "title": f"Sensitive Data in JWT: '{claim_key}'",
                    "detail": f"The claim '{claim_key}' may contain PII. JWTs are base64-encoded (NOT encrypted) and can be decoded by anyone.",
...
def audit_tokens_from_config(
    users_config: dict, wordlist_path: str = None
) -> Dict[str, Any]:
    ...
    for role, cfg in users_config.items():
        ...
        if token:
            if "." in token:
                ...
                logger.info(
                    f"[JWT Auditor] Audited token for role '{role}': {report[role].get('findings_count', 0)} findings"
                )
    return report
