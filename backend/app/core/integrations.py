        ...
        for res in results:
            if "VULNERABLE" in res['diagnosis']:
                payload["findings"].append({
                    ...
                    "cwe": int(res['cwe'].split(':')[0].replace('CWE-', '')) if 'CWE' in res['cwe'] else 284,
                    "mitigation": res.get('remediation', 'Implement strict RBAC/ABAC.')
                })
                
        if payload["findings"]:
            try:
                # In a real environment, this would hit the /api/v2/import-scan endpoint
                logger.info(f"[DefectDojo] Would push {len(payload['findings'])} findings to {self.url}")
