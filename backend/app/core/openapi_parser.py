...
class OpenAPIParser:
    ...
    def _validate_url_safe(self, url: str) -> None:
        ...
        if ip_obj.is_private:
            raise ValueError(f"SSRF Protection: Resolved IP {resolved_ip} is in a private range (RFC1918). Blocked.")
        if ip_obj.is_loopback:
            raise ValueError(f"SSRF Protection: Resolved IP {resolved_ip} is a loopback address. Blocked.")
        if ip_obj.is_reserved:
            raise ValueError(f"SSRF Protection: Resolved IP {resolved_ip} is a reserved address. Blocked.")
        if ip_obj.is_link_local:
            raise ValueError(f"SSRF Protection: Resolved IP {resolved_ip} is a link-local address. Blocked.")
        ...
        if resolved_ip in metadata_ips:
            raise ValueError(f"SSRF Protection: Resolved IP {resolved_ip} is a cloud metadata endpoint. Blocked.")
        
        logger.info(f"[SSRF Guard] URL validated: {hostname} -> {resolved_ip} (public, allowed)")
    ...
    def _validate_path_safe(self, path: str) -> str:
        ...
        if not os.path.isfile(canonical_path):
            raise ValueError(f"LFI Protection: Path '{canonical_path}' is not a file or does not exist.")
        
        logger.info(f"[LFI Guard] Path validated: {canonical_path} (within {ALLOWED_BASE_DIR})")
        ...
        try:
            if self.source.startswith("http://") or self.source.startswith("https://"):
                ...
                response = requests.get(
                    self.source, 
                    verify=False,  # TODO: Make configurable via --insecure flag
                    timeout=15,
