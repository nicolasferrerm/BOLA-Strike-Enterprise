...
class StateMachineFuzzer:
    ...
    def _resolve_template(self, template: Any, context: Dict[str, Any]) -> Any:
        ...
        if isinstance(template, dict):
            ...
            if (
                template.startswith("$ctx.")
                and len(template.split(".")) == 2
                and template.count("$ctx.") == 1
            ):
    ...
    def generate_fuzzing_scenarios(
        self, malicious_target_id: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i, node in enumerate(self.nodes):
            ...
            if i == critical_node_idx:
                ...
                payload = self._resolve_template(
                    step["payload_template"], {"target_account": malicious_target_id}
                )
                ...
                # Valid setup payload
                payload = self._resolve_template(
                    step["payload_template"], {"target_account": "valid_user_account"}
                )
        ...
        if len(self.nodes) > 1:
            ...
            attack_plan_2[0]["resolved_payload"] = self._resolve_template(
                self.nodes[-1]["payload_template"],
                {"target_account": malicious_target_id},
            )
                ...
                for skip_idx in range(1, critical_node_idx):
                    ...
                    for i, node in enumerate(self.nodes):
                        ...
                        if i == critical_node_idx:
                            step["is_attack_node"] = True
                            step["attack_type"] = (
                                f"BFLA_STATE_BYPASS_SKIP_STEP_{skip_idx}"
                            )
                            step["resolved_payload"] = self._resolve_template(
                            ...
                            step["resolved_payload"] = self._resolve_template(
                                step["payload_template"],
                                {"target_account": "valid_user_account"},
                            )
                        attack_plan_3.append(step)
                    scenarios.append(
                        {"scenario_id": str(uuid.uuid4()), "plan": attack_plan_3}
                    )
...
def build_banking_transfer_dag() -> StateMachineFuzzer:
    ...
    fuzzer.add_step(
        ...
        {"amount": 500.00, "currency": "CAD"},
        extract_vars=["transaction_id"],
    )
